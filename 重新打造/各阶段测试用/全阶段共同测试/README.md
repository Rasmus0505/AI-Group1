...
本目录用于运行前五个阶段的自动化验收：基础搭建、用户认证、房间系统基础、WebSocket 最小实时子系统、主持人初始化设定。提供组合脚本 `test-phase1-5.ps1`，内部按顺序调用阶段1/2、阶段3、阶段4、阶段5脚本。
...
> 如仅需阶段1/2，可运行 `.\test-phase1-2.ps1`；如需阶段1~3，可运行 `.\test-phase1-3.ps1`；如单独测试第四阶段，可进入 `../已完成4阶段测试` 运行 `.\test-phase4.ps1`；如单独测试第五阶段，可进入 `../5阶段测试` 运行 `.\test-phase5.ps1`。
