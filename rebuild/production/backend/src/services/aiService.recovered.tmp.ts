...
interface HistoryConfig {
  ...
  maxRounds: number;
  maxTokens: number;
  summarizeOldRounds: boolean;
}

// Lorebook entry (keyword-triggered long-term memory)
...
export interface InferenceRequest {
  ...
  gameRules: string;
  currentRound: number;
  // Game initialization data generated by host setup
  gameInitData?: {
...
export class AIService {
  ...
  private initTimeout = 300000; // 初始化任??分钟超时，与前端保持一致（保留你的改进??
  // 历史记忆默认配置
...
const rounds: ConversationMessage[][] = [];
    ...
    for (let i = rounds.length - 1; i >= 0; i--) {
      ...
      if (currentTokens + roundTokens <= maxTokens * 0.7) {
        // 还有空间，保留完整记�?        result.unshift(...roundMessages);
...
if (summaries.length > 0) {
  ...
  private summarizeRound(messages: ConversationMessage[]): string {
    ...
    // ??user 消息中提取回合号
    const roundMatch = userContent.match(/第\s*(\d+)\s*回�??);
    const roundNum = roundMatch ? roundMatch[1] : '?';

    // ??assistant 响应中提取关键事件（简化版??
let keyEvents = '';
    try {
      ...
      if (jsonMatch) {
        ...
        if (parsed.narrative) {
          // 提取叙事的�??00字作为摘??          keyEvents = keyEvents || parsed.narrative.substring(0, 100) + '...';
        }
      }
    } catch {
      // 解析失败，使用简单截�?      keyEvents = assistantContent.substring(0, 100) + '...';
    }

    return `??{roundNum}回�?? ${keyEvents}`;
  }

  /**
   * 粗略估算 token 数量（中文�??2 字�??token，英文约 4 字�??token??   */
...
const chineseChars = (text.match(/[\u4e00-\u9fa5]/g) || []).length;
  ...
  /**
   * 初始�?Lorebook（从游戏初始化数据生成）
   * Lorebook 是一种关键词触发式的长期记忆，当 prompt 中出现特定关键词时自动注入相关背??   */
  initializeLorebook(
    sessionId: string,
      ...
      if (gameInitData.entities) {
        gameInitData.entities.forEach((entity, index) => {
          entries.push({
            ...
            keywords: [entity.name, entity.id, `主�??{entity.id}`],
...
const quotedMatches = paragraph.match(/[「�??"]([^」�??"]+)[」�??"]/g);
      ...
      // 提取可能的公�?人名（连??-4个汉字�??
      const nameMatches = paragraph.match(/[\u4e00-\u9fa5]{2,4}(?:公司|集团|企业|工业|科技|贸�??/g);
      if (nameMatches) {
        keywords.push(...nameMatches);
      }

      return {
        keywords: [...new Set(keywords)].slice(0, 5), // 最??个关键词
...
if (entry.maxActivations && (entry.activationCount || 0) >= entry.maxActivations) {
      ...
      result.events.forEach((event: any, index: number) => {
        ...
        if (event.isLongTerm || event.type === 'negative' || event.type === 'positive') {
          this.addLorebookEntry(sessionId, {
            ...
            content: `【�??{round}回合事件??{event.description || event.keyword}`,
            priority: 30,
            enabled: true,
            category: 'event',
          });
  ...
  buildPrompt(request: InferenceRequest, gameRules: string): string {
    ...
    let prompt = `# 游戏推演请�??- ??${currentRound} 回合\n\n`;

    // 游戏规则：优先使用主持人配置�?gameRules，否则回退到蓝本默认规�?
if (gameRules && gameRules.trim()) {
      prompt += `## 游戏规则\n${gameRules.trim()} \n\n`;
    } else {
      // 蓝本版默认规则（简要），确保即使未配置也有合理的经营博弈逻�??
      prompt += '## 游戏规则（默�?- 凡墙皆是门蓝本）\n';
      prompt += '1. 回合与时间：每回合代表一个季度（3 个月），四个回合代表完整一年，可按�?0XX ??Q1」�??0XX ??Q2」等顺序推进。\n';
      ...
      prompt += '5. 现金流与破产：若现金余额小于必要支出（被动支??+ 本回合决策支出），视为现金流断裂，可能导致主体破产或丧失下一回合的决策权。\n';
    ...
    if (gameInitData) {
      ...
      // 主体信息（关键！告�??AI 主体的名称和初始状态�??
      if (gameInitData.entities && gameInitData.entities.length > 0) {
        prompt += `## 主体信息\n`;
        prompt += `本游戏共??${gameInitData.entities.length} 个决策主体，请严格使用以下主体名称和ID：\n\n`;
        gameInitData.entities.forEach((entity) => {
          prompt += `### 主�??${entity.id}??{entity.name}\n`;
          ...
          if (entity.backstory) {
            prompt += `- 背�?? ${entity.backstory}\n`;
          }
          prompt += `\n`;
        });
        prompt += `**重�??*: 在推演结果中，perEntityPanel 必须使用上述主体??id ??name，不要自行编造主体名称！\n\n`;
      }
      ...
      if (gameInitData.yearlyHexagram) {
        ...
        prompt += `## 年度卦象\n`;
        prompt += `- 卦�?? ${hex.name}\n`;
        prompt += `- 吉�?? ${hex.omen === 'positive' ? '?? : hex.omen === 'negative' ? '?? : '??}\n`;
        if (hex.lines) {
          prompt += `- 爻�?? ${hex.lines.join(' ')}\n`;
        }
        if (hex.text) {
          prompt += `- 象�?? ${hex.text}\n`;
        }
      ...
      activeEvents.forEach((event, index) => {
        ...
        if (event.progress && typeof event.progress === 'object') {
          ...
          if (progress.current !== undefined && progress.total !== undefined) {
            ...
            prompt += `   进�?? ${progress.current}/${progress.total} 回�??(${percentage}%)\n`;
            if (progress.lastUpdatedRound !== undefined) {
              prompt += `   上次更新: ??${progress.lastUpdatedRound} 回合\n`;
            }
          }
        }
        prompt += `   有效回合?? ${event.effectiveRounds}\n`;
        prompt += `   创建回合: ??${event.round} 回合\n`;
      });
      prompt += `\n`;
    } else {
      prompt += `## 活跃事件和规则\n（当前无活跃事件）\n\n`;
    ...
    // 玩家决策
    prompt += `## 玩家决策\n`;
    prompt += `**重�??*: 以下是本回合各主体玩家提交的实际决策，AI 必须基于这些决策内容进行推演，不要忽略或替换玩家的决策！\n\n`;
    if (decisions && decisions.length > 0) {
      decisions.forEach((decision) => {
        ...
        prompt += `### 主�??${decision.playerIndex} - ${entityName} 的决�?\n`;
        if (decision.actionText) {
          prompt += `  **玩家输入的行�?*: ${decision.actionText}\n`;
        }
        ...
        if (decision.hostModified && decision.hostModification) {
          prompt += `  **[主持人修�??*: ${JSON.stringify(decision.hostModification)}\n`;
        }
        prompt += `\n`;
      });
    ...
    // 推演要求：引导输出蓝本要求的结�??
    prompt += '## 推演要求\n';
    prompt += '请根据以上信息进行推演，**必须�?JSON 格式输出**，确保可被前端直接解析。\n\n';
    prompt += '### 输�??JSON Schema:\n';
    prompt += '```json\n';
    prompt += '{\n';
    prompt += '  "roundTitle": "第X回合�?0XX年�??下半�?,\n';
    prompt += '  "narrative": "叙事文本，描述本回合关键事件与因果链路，时间节奏按季�?半�??,\n';
    prompt += '  "events": [\n';
    prompt += '    {\n';
    prompt += '      "keyword": "事件关键??,\n';
    ...
    prompt += '      "name": "主体中文名称",\n';
    prompt += '      "cash": 1000000,\n';
    prompt += '      "attributes": { "市场份�??: 25, "品牌声誉": 70, "创新能力": 60 },\n';
    prompt += '      "passiveIncome": 50000,\n';
    prompt += '      "passiveExpense": 30000,\n';
    prompt += '      "delta": { "cash": 20000, "市场份�??: 2 },\n';
    prompt += '      "broken": false,\n';
    ...
    prompt += '        "description": "基于主体A本回合决策结果和当前状态生成的策略建�??,\n';
    prompt += '        "expectedDelta": { "cash": -50000, "市场份�??: 5 },\n';
    ...
    prompt += '        "description": "基于主体B本回合决策结果和当前状态生成的策略建�??,\n';
    ...
    prompt += '    ]\n';
    prompt += '  },\n';
    prompt += '  // 注意：perEntityOptions 为每个主体单独生??个策略选项，基于该主体本回合的实际决策结果\n';
    prompt += '  // expectedDelta ??cash 是实际金额（�?-50000 表示减少5万元），\n';
    prompt += '  // 其他属性是百分点变化（??市场份�?? 5 表示增加5个百分点，不�?%）\n';
    prompt += '  "riskCard": "企业风险简??,\n';
    prompt += '  "opportunityCard": "企业机会简??,\n';
    prompt += '  "benefitCard": "当前效益简??,\n';
    prompt += '  "achievements": [\n';
    ...
    prompt += '    { "entityId": "B", "message": "现金流警告信�?, "severity": "warning|critical" }\n';
    prompt += '  ]\n';
    prompt += '}\n';
    prompt += '```\n\n';
    prompt += '### 关键要求:\n';
    prompt += '1. **必须输出有�??JSON**，�??```json 代码块包裹\n';
    prompt += '2. **必须基于玩家提交的实际决策进行推??*，narrative 叙事中要体现玩家决策的执行和结果\n';
    prompt += '3. narrative 中应包含 events ??keyword，便于前端高亮\n';
    prompt += '4. perEntityPanel 数量必须与房间配置的主体数量一致\n';
    prompt += '5. 未收到指令的主体只结算被动收支，不触发主动事件\n';
    prompt += '6. 若余额接近被动支出临界，必须�?cashFlowWarning 中警示\n';
    prompt += '7. **perEntityOptions 必须为每个主体单独生�?个策略选�??*：\n';
    prompt += '   - 选项必须基于该主�?*本回合实际执行的决策结�??*来生成\n';
    prompt += '   - 不要把上回合的预设选项照搬，而是根据当前局势动态生成新选项\n';
    prompt += '   - 每个主体的选项应该反映其独特的处境、资源状况和战略机会\n';
    prompt += '   - 选项ID格式�?主体ID+序�??，如A1、A2、A3、B1、B2、B3\n';
    prompt += '8. 主�??id 使�??A/B/C/D，不要使??emoji\n';
    prompt += '9. **perEntityPanel.delta 是权威结算依�?*，请输出变化量；cash/attributes 绝对值仅作展示参考\n';
...
let requestBody = this.buildRequestBody(config, prompt, historyMessages, options);
    ...
    let axiosConfig: AxiosRequestConfig = {
      ...
      data: requestBody,
      timeout: timeout || this.defaultTimeout,  // 保留灵活的超时参�?      // 使用自定??Agent 增加连接稳定性（采用main分支的改进�??
      httpAgent: httpAgent,
    ...
    // 记录请求配置（不记录敏感信息�?    logger.info(`AI API request config prepared`, {
...
let errorMessage = error.message || 'Unknown error';
        ...
        if (error.response) {
          ...
          if (status === 401) {
            errorMessage = 'API密钥无效或已过期，请检查Authorization头配�?;
          } else if (status === 404) {
            errorMessage = 'API端点不存在，请检查endpoint配�??;
          } else if (status === 429) {
            errorMessage = 'API调用频率超限，请稍后重试';
          } else if (status >= 500) {
            errorMessage = `API服务器错??(${status})，请稍后重试`;
          } else {
            errorMessage = `API请求失败 (${status}): ${JSON.stringify(data)}`;
          }
        } else if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
          errorMessage = '无法连接到API服务器，请检查endpoint地址和网络连??;
        } else if (error.code === 'ETIMEDOUT') {
          errorMessage = '连接超时，请检查网络或增加超时时间';
        } else if (error.code === 'ECONNRESET') {
          errorMessage = '网络连接被重置，可能是网络不稳定或代理问题，正在重�??..';
        } else if (error.code === 'EPIPE' || error.code === 'EPROTO') {
          errorMessage = 'TLS/SSL连接异常，可能是网络环境问�??;
        } else if (error.message?.includes('socket') || error.message?.includes('TLS')) {
          errorMessage = '网络socket连接异常，可能是网络不稳??;
        }
...
if (provider === 'openai' || provider === 'deepseek' || provider === 'zhipu' || provider === 'default') {
    ...
    // 其他提供商的解析逻辑可以在这里添�?    // 例如：anthropic, google, custom??
    // 如果无法解析，尝试通用解�??
...
return {
  ...
  /**
   * 解析文本格式的推演结�?   * 支持??AI 响应中提??JSON 格式�?TurnResultDTO
   */
  ...
  private mapEntityPanelsToOutcomes(panels: any[] | undefined): Array<{
    ...
    return panels.map((panel, index) => ({
      ...
      outcome: panel.broken 
        ? `${panel.name} 已破�? ${panel.bankruptReason || '现金流断??}` 
...
const result = await this.callAI(
  ...
  async generateDecisionOptions(
    ...
    try {
      ...
      if (result.narrative) {
        const parsed = this.parseJsonFromText(result.narrative, 'decision_options');
        ...
        if (Array.isArray(rawOptions)) {
          optionsArray = rawOptions
            .map((option, index) => {
              ...
              return {
                option_id: String(record.option_id ?? index + 1),
                text: String(record.text ?? '').trim(),
                expected_effect: String(record.expected_effect ?? '').trim(),
                category: String(record.category ?? 'explore').trim(),
              };
            })
            .filter(option => option.text && option.expected_effect);
        } else if (parsed) {
          logger.warn(`Failed to parse JSON from narrative`, {
            reason: `Parsed payload does not contain options array`,
            parsedKeys: Object.keys(parsed),
          });
        }
      }
  ...
  async initializeGame(
    ...
    prompt += `- 游戏模式: ${initConfig.gameMode === 'multi_control' ? '多主体操控模�? : '单主角模�?}\n`;
    ...
    prompt += `1. **商业背景故�??*（�??00字）：\n`;
    prompt += `   - 用自然段落写小说故事，描�?${initConfig.entityCount} 个企业之间的博弈背景\n`;
    prompt += `   - 故事中不要讲卦象\n`;
    ...
    entityIds.forEach((id, index) => {
      prompt += `    {\n`;
      prompt += `      "id": "${id}",\n`;
      prompt += `      "name": "主�??{id}的中文名??,\n`;
      prompt += `      "cash": ${initConfig.initialCash},\n`;
      prompt += `      "attributes": { "市场份�??: 25, "品牌声誉": 70, "创新能力": 60 },\n`;
      prompt += `      "passiveIncome": 50000,\n`;
      prompt += `      "passiveExpense": 30000,\n`;
      prompt += `      "backstory": "主体背景简??\n`;
    ...
    try {
      ...
      // 如果解析失败，返回默认�??      logger.warn(`Failed to parse game initialization result, using defaults`);
  ...
  private getDefaultGameInit(initConfig: {
    ...
    const defaultNames = ['蓝鲸工业', '红隼贸易', '青龙科技', '白虎物�??, '玄武金�??, '朱雀传媒'];
    const entityIds = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.slice(0, initConfig.entityCount).split('');
    ...
    return {
      ...
      entities: entityIds.map((id, index) => ({
        id,
        name: defaultNames[index] || `企�??{id}`,
        cash: initConfig.initialCash,
        attributes: {
          '市场份�??: Math.floor(100 / initConfig.entityCount),
      ...
      initialOptions: [
        { id: '1', title: '扩大市场', description: '投入资金扩大市场份�??, expectedDelta: { cash: -50000, '市场份�??: 5 } },
        { id: '2', title: '研发创新', description: '投入研发提升创新能�??, expectedDelta: { cash: -30000, '创新能力': 10 } },
