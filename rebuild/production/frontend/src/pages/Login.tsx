...
import { UserOutlined, LockOutlined, LoginOutlined, ArrowLeftOutlined, WifiOutlined } from '@ant-design/icons';
...
function Login() {
  ...
  const [serverSelectorVisible, setServerSelectorVisible] = useState(false);
  ...
  return (
    ...
    >
                ...
                <Form.Item>
                  <div style={{ textAlign: 'center' }}>
                    <Button 
                      type="link" 
                      icon={<WifiOutlined />}
                      onClick={() => setServerSelectorVisible(true)}
                    >
                      局域网连接设置
                    </Button>
                  </div>
      ...
      {/* 服务器选择弹窗 */}
      <ServerSelector
        visible={serverSelectorVisible}
        onClose={() => setServerSelectorVisible(false)}
      />
    </div>
  );
}
