...
1.  **启动后端**: (为正式模式提供 API)
    ...
    *终端会显示当前运行模式: "Launching in LOCAL mode..." (默认) 或 CLOUD mode。*

3.  **启动云端连接 (可选)**: (仅 CLOUD 模式需要)
    - 双击运行根目录下的 `keep_alive_tunnel.bat`。
    - 输入 SSH 密码，保持窗口开启。
    - 启动后端时使用: `python main.py --cloud`
...
## 🌗 双存储模式 (Dual Storage Channels)
为了方便协作与测试，V3.0 引入了双通道存储架构：

### 1. 🟢 LOCAL 模式 (默认)
- **用途**: 团队成员开发、本地测试。
- **存储**: 本地文件 `local_game_data.json` / SQLite。
- **优点**: 无需联网，无需服务器权限，开箱即用。
- **启动**: 直接运行 `python main.py`。

### 2. ☁️ CLOUD 模式
- **用途**: 站长测试、生产环境模拟。
- **存储**: 阿里云 MongoDB (通过 CloudBridge)。
- **优点**: 数据上云，支持多端同步。
- **启动**: 
    1. 运行 `keep_alive_tunnel.bat` 建立隧道。
    2. 运行 `python main.py --cloud`。

> **提示**: 可通过 API `/api/toggle_mode` 在不重启的情况下动态切换模式。
...
### 3.4 逻辑链与因果循环 (The Causal Loop)
V3.0 的 `llm_engine.py` 实现了以下核心逻辑：
1. **Context**: 读取最近 30 条历史记录，防止 AI 健忘。
2. **Filters**: 检查玩家指令是否合规。
3. **Calculation**: 强制 AI 输出数值计算公式 (如 `Cash - 500`)。
4. **Narrative**: 生成基于剧情和随机卦象的故事。
5. **Output**: 返回 JSON 格式，包含 `resultNarrative` 和 `effects`。
